...
Referencing the above model, your public IP address (AKA IP) exists in layer 3, the Network layer. Your IP will be utilized to identify yourself and other computers in network. In essence, you are purchasing access to the internet through an ISP that acts as a courier service to send and receive data for you through the internetworked mesh that is the ‘internet.’ Further discussion on ISP particulars is not pertinent at this moment, suffice to say that telecommunications companies run high-capacity cables to coordinate web traffic between hosts and servers, utilizing a specialized L3 routing protocol called [Border Gateway Protocol](https://www.wikiwand.com/en/Border_Gateway_Protocol) (BGP). A tertiary concept you may have heard about is “[edge computing](https://www.wikiwand.com/en/Edge_computing#).” Per Wikipedia, “The origins of edge computing lie in [content distributed networks](https://www.wikiwand.com/en/Content_delivery_network) that were created in the late 1990s to serve web and video content from edge [servers](<https://www.wikiwand.com/en/Server_(computing)>) that are deployed closer to users for quick access.

Returning to our public IPv4 address, we may realize that one address is not enough for the many devices and users that exists in our household. That would be correct, thus our address must be shared somehow. A hardware device called a router can be introduced to solve this problem, and some others.
...
These “Network” protocols are algorithmic logic that are usually run-on ASIC hardware embedded, although software implementations exist. For example, three routing protocols are supported by the networking BSD OS [pfSense]( https://www.wikiwand.com/en/PfSense). In the next post, we will plan and set up a home network utilizing both a pfSense computer and a traditional [ASIC router/L3 switch]( https://mikrotik.com/product/RB2011UiAS-2HnD-IN).   
